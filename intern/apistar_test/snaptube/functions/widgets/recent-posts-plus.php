...
class RecentPostsPlus extends WP_Widget {
    ...
    private $default_config = array(
        ...
        'truncate_elipsis'           => '...',
        'post_thumbnail_width'       => '100',
        'post_thumbnail_height'      => '67',
        'post_date_format'           => 'd M Y',
        'wp_query_options'           => '',
        'widget_output_template'     => '
        <div class="news-item">
        <a href="{PERMALINK}" class="news-img no_thickbox">{THUMBNAIL}</a>
        <h3><a href="{PERMALINK}">{TITLE}</a></h3>
        <span class="comments icon-comment">{COMMENT_COUNT}</span>
        <span class="likes icon-heart">{LIKE_COUNT}</span>
        {EXCERPT}
        <span class="author icon-user">{AUTHOR_LINK}</span>
        <span class="date icon-calendar">{TIME_DIFF}</span>
    ...
    function __construct() {
        ...
        parent::__construct('recent-posts-plus', __('Snaptube - Recent Posts Plus', 'vh'), $widget_ops);
    }
        ...
        if( $the_query->have_posts() ) {
            ...
            while ( $the_query->have_posts() ) { $the_query->the_post();
                ...
                if( function_exists('snaptube_like_button') ) {
                    $like_func = snaptube_like_count( $ID );
                }

                $widget_ouput_template_params = array(
                    '{ID}'            => $ID,
                    '{THUMBNAIL}'     => preg_replace('#https?://#', '//', $POST_THUMBNAIL),
                    ...
                    '{LIKE_COUNT}'    => $like_func,
                    '{TIME_DIFF}'     => human_time_diff(get_the_time('U',$ID),current_time('timestamp')) .  ' ' . __('ago', 'vh')
                );
