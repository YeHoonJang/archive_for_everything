...
jQuery( document ).ready( function( $ ) {
	...
	*/
	...
	jQuery(document).on('click', '.snaptube-rating-dismiss', function() {
		jQuery.ajax({
			type: 'POST',
			url: ajaxurl,
			data: { 
				'action': 'snaptube_dismiss_notice'
			},
			success: function(data) {
				jQuery('.snaptube-rating-notice').remove();
			}
		});
	});

	if ( jQuery('.youtubelinkinput').length && jQuery('.youtubelinkinput').val().indexOf('youtube') >= 0 ) {
		jQuery('[name="youtube_media"]').before('<a href="javascript:void(0)" class="snaptube-reload-images button-primary">Delete thumbnails</a>');
	}

	jQuery(document).on('click', '.snaptube-reload-images', function() {
		...
		jQuery.ajax({
			...
			url: ajaxurl,
			data: { 
				'action': 'snaptube_reload_images',
				'video_id': full_url.split('videoId=')['1']
			},
			success: function(data) {
...
jQuery(document).ready(function() {
	...
	jQuery('.font-family').change(function() {
		...
		if (google_font) {
			...
			link.rel = "stylesheet";
			link.type = "text/css";
			link.href = '//fonts.googleapis.com/css?family=' + jQuery(this).val() + '&subset=cyrillic,greek,latin';
		}
