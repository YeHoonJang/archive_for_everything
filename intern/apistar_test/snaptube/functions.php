...
define('VH_DEVELOPER_URL', 'https://cohhe.com');
...
if (!function_exists('vh_setup')) {
	...
	function vh_setup() {
		...
		if ( !is_admin() && defined('APPTHA_VGALLERY_BASEURL') ) {
...
function vh_imgresize($url, $width, $height = null, $postid, $crop = null, $single = true) {
	...
	$video_upload_folder = wp_upload_dir( get_the_date( 'Y/m', $postid) );

	//check if $img_url is local
	if ( file_exists($video_upload_folder['path'].'/video-'. $image_quality . '-' . $postid . '.jpg') ) {
		return preg_replace('#^https?://#', '//', $video_upload_folder['url'].'/video-'. $image_quality . '-' . $postid . '.jpg');
	} elseif (strpos($url, $upload_url) === false) {
		$url = vh_media_sideload_image($url, $postid, '', 'src', $image_quality);

		if ( is_wp_error($url) ) {
			return preg_replace('#^https?://#', '//', get_template_directory_uri().'/images/nothumbimage.jpg');
		}
	}
...
function vh_media_sideload_image( $file, $post_id, $desc = null, $return = 'html', $image_type = '' ) {
	...
	if ( ! empty( $file ) ) {
		...
		$new = $tmppath['dirname'] . "/video-". $image_type . "-" . $post_id . "." . $tmppath['extension'];          // build new path
		rename($tmp, $new);                                                                 // renames temp file on server
		$tmp = $new;                                                                        // push new filename (in path) to be used in file array later

		// assemble file data (should be built like $_FILES since wp_handle_sideload() will be using)
		$file_array['tmp_name'] = $tmp;
		$file_array['name'] = "video-". $image_type . "-" . $post_id . "." . $url_type['ext'];
	...
	if ( $video_ids != null || $video_ids != '' ) {
		...
		foreach ($suggested_video as $video) {
			...
			$output .= '<div class="video_image_container '.$video_sd_image.'">
							...
							<img src="' . preg_replace('#^https?://#', '//', $imageFea) . '" alt="' . $video->name . '" class="related" />
							<div id="video_dialog" title="' . $video->name . '">';
							if ( get_option('vh_html5_videos', 'true') == 'false' || get_option('vh_html5_videos', 'true') == false ) {
								if( $file_type == 5 && !empty($video->embedcode) ) {
									...
									if($mobile === true){
										...
										if (preg_match("/www\.youtube\.com\/watch\?v=[^&]+/", $reafile, $vresult)) {
											...
											$reavideourl = "//www.youtube.com/embed/$video_id";
											...
											if ($file_type == 4) {           ## For RTMP videos
												$streamer = str_replace("rtmp://", "//", $media->streamer_path);
			...
			$output .= '<div class="video_info">';
				...
				if ( function_exists('snaptube_like_button') ) {
					$output .= '<div class="video_likes icon-heart">'. snaptube_like_count( $video->slug ) . '</div>'; 
				}
				$output .= '</div>
	...
	if ( $post_image != '' ) {
		...
		<div class="post-thumb">
			<div class="post-thumb-img-wrapper">
				...
				<a href="'.get_permalink($post_id).'" class="link_image" title="Permalink to '.get_the_title($post_id).'">'.preg_replace('#https?://#', '//', $post_image).'</a>
			</div>
			<div class="post-info-box">
				<div class="vc_post_comments icon-comment">' . $tc->total_comments . '</div>';
				if ( function_exists('snaptube_like_button') ) {
					$output .= '<div class="vc_post_likes icon-heart">' . snaptube_like_count( $post_id ) . '</div>';
				}
			$output .= '
			</div>
		</div>';
...
function tgm_cpt_search( $query ) {
	if ( $query->is_search ) {
		$include_in_search = array( 'page', 'post' );
		if ( defined('APPTHA_VGALLERY_BASEURL') ) {
			$include_in_search[] = 'videogallery';
		}
		$query->set( 'post_type', $include_in_search );
	}
	return $query;
}
...
if (!function_exists('vh_scripts_method')) {
	function vh_scripts_method() {
		...
		wp_enqueue_script('master', get_template_directory_uri() . '/js/master.js', array('jquery', 'prettyphoto', 'isotope'), '', TRUE);
		wp_enqueue_script('isotope', get_template_directory_uri() . '/js/jquery.isotope.min.js', array(), '', TRUE);
		wp_enqueue_script('isotope.layout', get_template_directory_uri() . '/js/isotope-masonry-horizontal.js', array('isotope'), '', TRUE);
...
function vh_check_followed_categories() {
	global $wpdb;
	if ( is_user_logged_in() && defined('APPTHA_VGALLERY_BASEURL') && get_user_meta(get_current_user_id(), 'followed_video_categories', true) != ''   ) {
		$user_followed_categories = json_decode(get_user_meta(get_current_user_id(), 'followed_video_categories', true), true);
...
function vh_register_required_plugins() {
	...
	$plugins = array(
		array(
			...
			'version'            => '2.2.7', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
...
function vh_import_videos() {
	...
	$wpdb->query("INSERT INTO " . $table_name . " (`vid`, `name`, `description`, `embedcode`, `file`, `streamer_path`, `hdfile`, `slug`, `file_type`, `duration`, `srtfile1`, `srtfile2`, `subtitle_lang1`, `subtitle_lang2`, `image`, `opimage`, `download`, `link`, `featured`, `hitcount`, `ratecount`, `rate`, `post_date`, `postrollads`, `prerollads`, `midrollads`, `imaad`, `publish`, `islive`, `member_id`, `ordering`) VALUES
				(34, 'Happy people in Tatras', 'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters.', '', 'https://www.youtube.com/watch?v=sGrHwBlf-7M', '', '', '67', 1, '4:04', '', '', '', '', 'https://i3.ytimg.com/vi/sGrHwBlf-7M/mqdefault.jpg', 'https://i3.ytimg.com/vi/sGrHwBlf-7M/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=sGrHwBlf-7M', '1', 1473, 0, 0, '2014-05-04 08:21:45', '1', '1', 1, 0, 1, 0, " . $member_id . ", 13),
				(35, '3D Animated Short Film', 'This short computer animated film was created by the Blender Institute, part of the Blender Foundation. Like the foundation''s previous film Elephants Dream, the film was made using Blender, a free software application for animation made by the same foundation.', '', 'https://www.youtube.com/watch?v=AcBut-jY0G8', '', '', '68', 1, '10:22', '', '', '', '', 'https://i3.ytimg.com/vi/AcBut-jY0G8/mqdefault.jpg', 'https://i3.ytimg.com/vi/AcBut-jY0G8/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=AcBut-jY0G8', '1', 105, 0, 0, '2014-05-04 08:27:14', '0', '0', 0, 0, 1, 0, " . $member_id . ", 14),
				(32, 'Antarctic Mountain Climbing', 'Accomplished climber Mike Libecki and photographer Cory Richards battle extreme cold and furious katabatic winds in an epic, ten-day climb to the summit of the untouched Bertha''s Tower at the bottom of the world.', '', 'https://www.youtube.com/watch?v=EULc7RgnM4c', '', '', '65', 1, '23:12', '', '', '', '', 'https://i3.ytimg.com/vi/EULc7RgnM4c/mqdefault.jpg', 'https://i3.ytimg.com/vi/EULc7RgnM4c/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=EULc7RgnM4c', '1', 72, 0, 0, '2014-05-04 08:05:21', '0', '0', 0, 0, 1, 0, " . $member_id . ", 12),
				(31, 'Go Where the Locals Go', 'In \"Where the Locals Go,\" you''ll find hundreds of beautifully photographed travel experiences, with nuggets of entertaining and insightful text informed by locals. Leave the tourist trail behind, and make your next trip truly authentic!', '', 'https://www.youtube.com/watch?v=DoCTVac9kn4', '', '', '64', 1, '1:31', '', '', '', '', 'https://i3.ytimg.com/vi/DoCTVac9kn4/mqdefault.jpg', 'https://i3.ytimg.com/vi/DoCTVac9kn4/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=DoCTVac9kn4', '1', 44, 0, 0, '2014-05-04 08:03:35', '0', '0', 0, 0, 1, 0, " . $member_id . ", 11),
				(30, 'Telling the Story', 'Get the inside story behind crafting the text in the world''s best known magazine directly from its renowned writers and editors.\r\n\r\nThe National Geographic Live! series brings thought-provoking presentations by today''s leading explorers, scientists, photographers, and performing artists right to your YouTube feed. Each presentation is filmed in front of a live audience at National Geographic headquarters in Washington, D.C. New clips air every Monday.', '', 'https://www.youtube.com/watch?v=r8YeqTTzBFo', '', '', '63', 1, '25:37', '', '', '', '', 'https://i3.ytimg.com/vi/r8YeqTTzBFo/mqdefault.jpg', 'https://i3.ytimg.com/vi/r8YeqTTzBFo/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=r8YeqTTzBFo', '1', 242, 0, 0, '2014-05-04 08:02:40', '0', '0', 0, 0, 1, 0, " . $member_id . ", 10),
				(29, 'The Aboriginal Homeland', 'Through her stunning photography, Amy Toensing touches upon the Aboriginal Australians'' cultural struggle, but celebrates these indigenous people''s unique way of life and their connection to their ancestral lands.\r\n\r\nUpcoming Events at National Geographic Live!\r\nhttps://events.nationalgeographic.com/events/\r\n\r\nThe National Geographic Live! series brings thought-provoking presentations by today''s leading explorers, scientists, photographers, and performing artists right to your YouTube feed. Each presentation is filmed in front of a live audience at National Geographic headquarters in Washington, D.C. New clips air every Monday.', '', 'https://www.youtube.com/watch?v=Y4PMZtRYmMs', '', '', '62', 1, '21:24', '', '', '', '', 'https://i3.ytimg.com/vi/Y4PMZtRYmMs/mqdefault.jpg', 'https://i3.ytimg.com/vi/Y4PMZtRYmMs/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=Y4PMZtRYmMs', '1', 42, 0, 0, '2014-05-04 08:01:19', '0', '0', 0, 0, 1, 0, " . $member_id . ", 9),
				(85, 'Read the Heart Line', 'Learn how to find and read the heart line from psychotherapist and palm reading expert Ellen Goldberg, M.A. in this Howcast video.\r\n\r\nThere are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don''t look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn''t anything embarrassing hidden in the middle of text. All the Lorem Ipsum generators on the Internet tend to repeat predefined chunks as necessary, making this the first true generator on the Internet.', '', 'https://www.youtube.com/watch?v=3yKjCZP7KUQ', '', '', '131', 1, '5:20', '', '', '', '', 'https://i3.ytimg.com/vi/3yKjCZP7KUQ/mqdefault.jpg', 'https://i3.ytimg.com/vi/3yKjCZP7KUQ/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=3yKjCZP7KUQ', '1', 18, 0, 0, '2014-05-07 14:37:33', '0', '0', 0, 0, 1, 0, " . $member_id . ", 54),
				(28, 'The Reindeer People', 'Spending years in the field, photographer Erika Larsen has gained unprecedented access into the lives, work, and culture of Scandinavia''s fascinating Sami people.\r\n\r\nThe National Geographic Live! series brings thought-provoking presentations by today''s leading explorers, scientists, photographers, and performing artists right to your YouTube feed. Each presentation is filmed in front of a live audience at National Geographic headquarters in Washington, D.C. New clips air every Monday.', '', 'https://www.youtube.com/watch?v=bPiKAhhEHXA', '', '', '61', 1, '22:41', '', '', '', '', 'https://i3.ytimg.com/vi/bPiKAhhEHXA/mqdefault.jpg', 'https://i3.ytimg.com/vi/bPiKAhhEHXA/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=bPiKAhhEHXA', '1', 1108, 0, 0, '2014-05-04 08:00:14', '0', '0', 0, 0, 1, 0, " . $member_id . ", 8),
				(27, 'Running Up Hill', 'Paulie and Tuffy scout a very difficult location to build a cabin.\r\n\r\nLorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sit amet fringilla orci. Aenean molestie dolor nibh, non accumsan orci fermentum vitae. Proin bibendum at diam tincidunt fermentum. Nunc venenatis volutpat erat, sit.', '', 'https://www.youtube.com/watch?v=l3qUvdy1Dh8', '', '', '60', 1, '53:48', '', '', '', '', 'https://i3.ytimg.com/vi/l3qUvdy1Dh8/mqdefault.jpg', 'https://i3.ytimg.com/vi/l3qUvdy1Dh8/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=l3qUvdy1Dh8', '0', 19, 0, 0, '2014-05-04 07:59:42', '0', '0', 0, 0, 1, 0, " . $member_id . ", 7),
				(23, 'Lightweight', 'Tim makes it REALLY difficult for a body builder to pick up a woman. NONE OF THE ABOVE AIRS MONDAYS at 10P.', '', 'https://www.youtube.com/watch?v=y_eLZr24iMQ', '', '', '56', 1, '12:43', '', '', '', '', 'https://i3.ytimg.com/vi/y_eLZr24iMQ/mqdefault.jpg', 'https://i3.ytimg.com/vi/y_eLZr24iMQ/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=y_eLZr24iMQ', '0', 32, 0, 0, '2014-05-04 07:55:09', '0', '0', 0, 0, 1, 0, " . $member_id . ", 4),
				(25, 'Replacing Florida''s Stolen Orchids', 'Wild orchids in South Florida were all but eliminated by humans turning them into disposable potted plants, beginning in the 1800s. A comeback is difficult because orchid seeds have only about a one-in-a-million chance of creating a new plant. But there''s an effort to bring the wild orchid population back to its former glory.\r\n\r\nPRODUCER: Gabriella Garcia-Pardo', '', 'https://www.youtube.com/watch?v=jntK5b-BwXI', '', '', '58', 1, '3:50', '', '', '', '', 'https://i3.ytimg.com/vi/jntK5b-BwXI/mqdefault.jpg', 'https://i3.ytimg.com/vi/jntK5b-BwXI/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=jntK5b-BwXI', '0', 8, 0, 0, '2014-05-04 07:56:18', '0', '0', 0, 0, 1, 0, " . $member_id . ", 5),
				(26, 'Foiled by a Balloon', 'What will happen if Tim puts a foil balloon into a microwave?  NONE OF THE ABOVE AIRS MONDAYS at 10P.', '', 'https://www.youtube.com/watch?v=5S4uZDMOQ7A', '', '', '59', 1, '1:06', '', '', '', '', 'https://i3.ytimg.com/vi/5S4uZDMOQ7A/mqdefault.jpg', 'https://i3.ytimg.com/vi/5S4uZDMOQ7A/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=5S4uZDMOQ7A', '0', 14, 0, 0, '2014-05-04 07:57:25', '0', '0', 0, 0, 1, 0, " . $member_id . ", 6),
				(17, 'Big Buck Bunny', 'Big Buck Bunny was the first project in the Blender Institute Amsterdam. This 10 minute movie has been made inspired by the best cartoon tradition.', '', 'https://www.youtube.com/watch?v=Vpg9yizPP_g', '', '', '23', 1, '1:47', '', '', '', '', 'https://i3.ytimg.com/vi/Vpg9yizPP_g/mqdefault.jpg', 'https://i3.ytimg.com/vi/Vpg9yizPP_g/maxresdefault.jpg', '', 'https://www.youtube.com/watch?v=Vpg9yizPP_g', '1', 94, 0, 0, '2013-08-06 13:53:12', '0', '0', 0, 0, 1, 0, " . $member_id . ", 16),
				(22, 'Kayapo Warrior Tribe', 'Portrait photographer Martin Schoeller travels to the remote Brazilian Amazon where the Kayapo people balance the colorful traditions of their heritage and the enticing commodities of the 21st century.\r\n\r\nThe National Geographic Live! series brings thought-provoking presentations by today''s leading explorers, scientists, photographers, and performing artists right to your YouTube feed. Each presentation is filmed in front of a live audience at National Geographic headquarters in Washington, D.C. New clips air every Monday.', '', 'https://www.youtube.com/watch?v=tDuK5Axe0-Y', '', '', '55', 1, '20:43', '', '', '', '', 'https://i3.ytimg.com/vi/tDuK5Axe0-Y/mqdefault.jpg', 'https://i3.ytimg.com/vi/tDuK5Axe0-Y/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=tDuK5Axe0-Y', '0', 115, 0, 0, '2014-05-04 07:53:57', '0', '0', 0, 0, 1, 0, " . $member_id . ", 3),
				(21, 'Exploring the Cosmic Dawn', 'Scientists are getting an unprecedented view of the early universe with the help of ALMA, the Atacama Large Millimeter/submillimeter Array. This group of antennae is the equivalent of a city-size telescopic lens, bringing cosmic nurseries for planets and stars into view.\r\n\r\nLearn more about ALMA and the cosmic dawn:\r\nhttps://www.nationalgeographic.com/cosmic-dawn/\r\n\r\nPHOTOGRAPHY: Dave Yoder, Peter Wintersteller, and Pilar Elorriaga\r\nEDITOR: Hans Weise\r\nMUSIC: John Kusiak', '', 'https://www.youtube.com/watch?v=4I8KJFIuHKE', '', '', '54', 1, '1:15', '', '', '', '', 'https://i3.ytimg.com/vi/4I8KJFIuHKE/mqdefault.jpg', 'https://i3.ytimg.com/vi/4I8KJFIuHKE/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=4I8KJFIuHKE', '0', 3, 0, 0, '2014-05-04 07:50:47', '0', '0', 0, 0, 1, 0, " . $member_id . ", 4),
				(20, 'Window to Your Health', 'Your eyes are tiny spheres of wonder. A doctor can find warning signs of high blood pressure, diabetes, and a whole range of other systemic health issues, just by examining your eyes. Ophthalmologist Neal Adams explains why the eye''s tissues and blood vessels make such a good barometer for wellness.', '', 'https://www.youtube.com/watch?v=BPAbANevTqM', '', '', '53', 1, '2:50', '', '', '', '', 'https://i3.ytimg.com/vi/BPAbANevTqM/mqdefault.jpg', 'https://i3.ytimg.com/vi/BPAbANevTqM/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=BPAbANevTqM', '1', 14, 0, 0, '2014-05-04 07:46:35', '0', '0', 0, 0, 1, 0, " . $member_id . ", 19),
				(36, 'Robinson Crusoe', 'This funny animation short movie was created by several students for their graduation project at ESMA (Montpellier, France).', '', 'https://www.youtube.com/watch?v=TKoB7XQJwlo', '', '', '69', 1, '2:27', '', '', '', '', 'https://i3.ytimg.com/vi/TKoB7XQJwlo/mqdefault.jpg', 'https://i3.ytimg.com/vi/TKoB7XQJwlo/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=TKoB7XQJwlo', '1', 150, 0, 0, '2014-05-04 08:28:55', '1', '1', 1, 0, 1, 0, " . $member_id . ", 15),
				(40, 'Ratatouille', 'Disney''s Ratatouille official movie game LIKE for more videos.', '', 'https://www.youtube.com/watch?v=BV1C4ZNtx2M', '', '', '73', 1, '2:7:42', '', '', '', '', 'https://i3.ytimg.com/vi/BV1C4ZNtx2M/mqdefault.jpg', 'https://i3.ytimg.com/vi/BV1C4ZNtx2M/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=BV1C4ZNtx2M', '1', 39, 0, 0, '2014-05-04 08:57:24', '0', '0', 0, 0, 1, 0, " . $member_id . ", 19),
				(37, 'Caminandes', 'Open Movie produced by Blender Institute - 3D Animated Short Film HD Read more about the Caminandes series on https://www.example.com', '', 'https://www.youtube.com/watch?v=yVh_e_BswDE', '', '', '70', 1, '2:27', '', '', '', '', 'https://i3.ytimg.com/vi/yVh_e_BswDE/mqdefault.jpg', 'https://i3.ytimg.com/vi/yVh_e_BswDE/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=yVh_e_BswDE', '1', 46, 0, 0, '2014-05-04 08:40:34', '0', '0', 0, 0, 1, 0, " . $member_id . ", 16),
				(38, 'Dum Spiro - 3D Animation', 'Dum Spiro is a full 3D animated cartoon movie HD, this animation short film uses Maya created by several students for their graduation project at ESMA (Montpellier, France).', '', 'https://www.youtube.com/watch?v=GJwKoIbBkZs', '', '', '71', 1, '6:22', '', '', '', '', 'https://i3.ytimg.com/vi/GJwKoIbBkZs/mqdefault.jpg', 'https://i3.ytimg.com/vi/GJwKoIbBkZs/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=GJwKoIbBkZs', '1', 33, 0, 0, '2014-05-04 08:41:09', '0', '0', 0, 0, 1, 0, " . $member_id . ", 17),
				(39, 'Animation Movie - Sintel', 'Sintel is the third animation movie produced by the Blender Foundation. This 3D animated short film was directed by Savannah College of Art and Design Student, Colin Levy.', '', 'https://www.youtube.com/watch?v=W3sNH41SeoM', '', '', '72', 1, '3:15', '', '', '', '', 'https://i3.ytimg.com/vi/W3sNH41SeoM/mqdefault.jpg', 'https://i3.ytimg.com/vi/W3sNH41SeoM/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=W3sNH41SeoM', '1', 148, 0, 0, '2014-05-04 08:41:34', '0', '0', 0, 0, 1, 0, " . $member_id . ", 18),
				(41, 'Animation Movie - FAT', 'FAT is a funny animation movie hd made by 3 students in 2011, the film received many selections and awards, please read the description for more details. Feel free to Like or Share the film!', '', 'https://www.youtube.com/watch?v=yltlJEdSAHw', '', '', '74', 1, '2:02', '', '', '', '', 'https://i3.ytimg.com/vi/yltlJEdSAHw/mqdefault.jpg', 'https://i3.ytimg.com/vi/yltlJEdSAHw/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=yltlJEdSAHw', '1', 52, 0, 0, '2014-05-04 08:58:39', '0', '0', 0, 0, 1, 0, " . $member_id . ", 20),
				(42, 'The Smurfs 2', 'The Smurfs 2 Full Movie Game - Video for Kids - Spongebob Squarepants TV! Dora the Explorer TV - Games for Kids 2013 :)', '', 'https://www.youtube.com/watch?v=vQbSGLaVJ5c', '', '', '75', 1, '1:34', '', '', '', '', 'https://i3.ytimg.com/vi/vQbSGLaVJ5c/mqdefault.jpg', 'https://i3.ytimg.com/vi/vQbSGLaVJ5c/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=vQbSGLaVJ5c', '1', 56, 0, 0, '2014-05-04 09:00:33', '0', '0', 0, 0, 1, 0, " . $member_id . ", 21),
				(43, 'Baby Hazel Games', 'Baby Hazel Games - Baby and Kids Games Movie Dora Games for Kids - Dora the Explorer. Kids PC Games Cartoons for Babies. There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour.', '', 'https://www.youtube.com/watch?v=JuyB7NO0EYY', '', '', '76', 1, '4:59', '', '', '', '', 'https://i3.ytimg.com/vi/JuyB7NO0EYY/mqdefault.jpg', 'https://i3.ytimg.com/vi/JuyB7NO0EYY/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=JuyB7NO0EYY', '1', 38, 0, 0, '2014-05-04 09:01:43', '0', '0', 0, 0, 1, 0, " . $member_id . ", 22),
				(44, 'Alice Amv', 'Alice Amv - Alice in Wonderland Full Movie Game 2014 - Full Alice tribute, Gameplays & Trailers Alice Madness Returns.', '', 'https://www.youtube.com/watch?v=yf1trNG2PE0', '', '', '77', 1, '3:57', '', '', '', '', 'https://i3.ytimg.com/vi/yf1trNG2PE0/mqdefault.jpg', 'https://i3.ytimg.com/vi/yf1trNG2PE0/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=yf1trNG2PE0', '1', 53, 0, 0, '2014-05-04 09:02:24', '0', '0', 0, 0, 1, 0, " . $member_id . ", 23),
				(45, 'Cupidon', 'Animation Movie Cupidon is a Animation Short Film HD. This short movie was created by several students for their graduation project at ESMA (Montpellier, France).', '', 'https://www.youtube.com/watch?v=gJ7xINoeFPk', '', '', '78', 1, '3:42', '', '', '', '', 'https://i3.ytimg.com/vi/gJ7xINoeFPk/mqdefault.jpg', 'https://i3.ytimg.com/vi/gJ7xINoeFPk/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=gJ7xINoeFPk', '1', 42, 0, 0, '2014-05-04 09:03:46', '0', '0', 0, 0, 1, 0, " . $member_id . ", 24),
				(46, 'Visit to the Leather Maker', 'Mick has a hard time bartering a tree burl for some leather goods.  THE LEGEND OF MICK DODGE AIRS TUESDAYS at 10P.', '', 'https://www.youtube.com/watch?v=hbuMIj9wrH0', '', '', '79', 1, '2:19', '', '', '', '', 'https://i3.ytimg.com/vi/hbuMIj9wrH0/mqdefault.jpg', 'https://i3.ytimg.com/vi/hbuMIj9wrH0/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=hbuMIj9wrH0', '1', 28, 0, 0, '2014-05-04 14:01:10', '0', '0', 0, 0, 1, 0, " . $member_id . ", 25),
				(47, '3D Short Film', 'Out of Bounds is a graduation film made at The Animation Workshop, Denmark, by Viktoria Piechowitz, Andreea Jebelean, Adrian Walt, Stephen C. Brossman, Laika A. Laursen, Natalia Marcos, Ida Mårtenson, Martin Zauner, Kirsten Bay Nielsen, Oldrich Holy, and Signe T. Schmidt.', '', 'https://www.youtube.com/watch?v=j6PbonHsqW0', '', '', '80', 1, '3:36', '', '', '', '', 'https://i3.ytimg.com/vi/j6PbonHsqW0/mqdefault.jpg', 'https://i3.ytimg.com/vi/j6PbonHsqW0/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=j6PbonHsqW0', '1', 72, 0, 0, '2014-05-04 14:03:28', '0', '0', 0, 0, 1, 0, " . $member_id . ", 26),
				(48, 'Ice Age 4', 'Arctic Games is a video game based on the film of the same name. You need to pick your team. If you pick Manny''s team,you will play as Ellie first (tunnel sliding race), then with Sid against Squint (long jump and glyptodon throw), Granny against Gupta (outdoor sliding race), Diego against Shira (jumping race), Peaches against Raz (slingshot target) and Manny against Gutt in another tunnel sliding game where you must smash the ice with the correct team sign.', '', 'https://www.youtube.com/watch?v=rWxJlfaaKWc', '', '', '81', 1, '10:52', '', '', '', '', 'https://i3.ytimg.com/vi/rWxJlfaaKWc/mqdefault.jpg', 'https://i3.ytimg.com/vi/rWxJlfaaKWc/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=rWxJlfaaKWc', '1', 51, 0, 0, '2014-05-04 14:05:55', '0', '0', 0, 0, 1, 0, " . $member_id . ", 27),
				(51, 'Electroshock', 'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.\r\n\r\nCreated, Directed & Animated by Hugo Jackson, Pascal Chandelier, Valentin Michel, Bastien MORTELEcque and Elliot Maren. With the voices of Christophe Lemoine, Ariane Aggiage, Michel Vigne, Laetitia Barbara, Philipe Peythieu and Véronique Augereau. Music by Thierry Jaoul, Jose Vicente and Hugo Jackson. Sound Design by José Vicente and Yoann Poncet, Studio des Aviateurs.', '', 'https://www.youtube.com/watch?v=hRHbeYZhtgA', '', '', '84', 1, '8:43', '', '', '', '', 'https://i3.ytimg.com/vi/hRHbeYZhtgA/mqdefault.jpg', 'https://i3.ytimg.com/vi/hRHbeYZhtgA/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=hRHbeYZhtgA', '1', 49, 0, 0, '2014-05-04 14:11:40', '0', '0', 0, 0, 1, 0, " . $member_id . ", 29),
				(52, 'Tolerantia', 'Animals are eukaryotic organisms of the kingdom Animalia or Metazoa. Their body the first 3D animated film produced in Bosnia and Herzegovina. Nominated for the Best European Short film by the European Film Academy 2008. Awarded with 8 more awards including the \"Heart of Sarajevo\" for the best short film on Sarajevo Film Festival 2008.\r\nDirecting, animation, sound - Ivan Ramadan\r\nMusic - traditional (Mostar Sevdah Reunion)', '', 'https://www.youtube.com/watch?v=FrjQrXc80cY', '', '', '85', 1, '6:21', '', '', '', '', 'https://i3.ytimg.com/vi/FrjQrXc80cY/mqdefault.jpg', 'https://i3.ytimg.com/vi/FrjQrXc80cY/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=FrjQrXc80cY', '1', 34, 0, 0, '2014-05-04 14:13:11', '0', '0', 0, 0, 1, 0, " . $member_id . ", 30),
				(53, 'Extraordinary Skies', 'One year—and more than 12,000 photos—in the making, \"Into the Atmosphere\" is photographer Michael Shainblum''s homage to the natural wonders of his native California.', '', 'https://www.youtube.com/watch?v=vLUNWYt3q1w', '', '', '86', 1, '3:46', '', '', '', '', 'https://i3.ytimg.com/vi/vLUNWYt3q1w/mqdefault.jpg', 'https://i3.ytimg.com/vi/vLUNWYt3q1w/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=vLUNWYt3q1w', '1', 7, 0, 0, '2014-05-04 14:40:22', '0', '0', 0, 0, 1, 0, " . $member_id . ", 31),
				(54, 'Norway Northern Lights', 'March 21, 2012 — Multicolored curtains of light fill the skies over northern Norway in a new time-lapse video made from aurora images taken this month. Filmmakers Claus and Anneliese Possberg used about 600 frames to create the video.', '', 'https://www.youtube.com/watch?v=izYiDDt6d8s', '', '', '89', 1, '4:26', '', '', '', '', 'https://i3.ytimg.com/vi/izYiDDt6d8s/mqdefault.jpg', 'https://i3.ytimg.com/vi/izYiDDt6d8s/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=izYiDDt6d8s', '1', 4, 0, 0, '2014-05-04 14:45:49', '0', '0', 0, 0, 1, 0, " . $member_id . ", 32),
				(55, 'Elemental Iceland', 'Photographer Stian Rekdal combined thousands of photos to create this time-lapse video showcasing Iceland''s natural beauty. He spent three weeks—and more than 3,000 miles—on the road and took more than 40,000 photos. He used about 3,500 of these to make the video.', '', 'https://www.youtube.com/watch?v=_vhf0RZg0fg', '', '', '90', 1, '2:47', '', '', '', '', 'https://i3.ytimg.com/vi/_vhf0RZg0fg/mqdefault.jpg', 'https://i3.ytimg.com/vi/_vhf0RZg0fg/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=_vhf0RZg0fg', '1', 5, 0, 0, '2014-05-04 15:40:05', '0', '0', 0, 0, 1, 0, " . $member_id . ", 33),
				(56, 'Commandments', 'Uploaded by Lokesh Bade. This channel is a Non-Profit one and all the videos are solely for ministry purposes only. The life of Moses from birth to receiving The Ten Commandments from The Lord God.\r\nAlmost close to The Holy scriptures.\r\n\r\nIt is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using ''Content here, content here'', making it look like readable English.', '', 'https://www.youtube.com/watch?v=hPovrwqaErs', '', '', '91', 1, '1:43', '', '', '', '', 'https://i3.ytimg.com/vi/hPovrwqaErs/mqdefault.jpg', 'https://i3.ytimg.com/vi/hPovrwqaErs/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=hPovrwqaErs', '1', 142, 0, 0, '2014-05-06 12:58:20', '0', '0', 0, 0, 1, 0, " . $member_id . ", 34),
				(57, 'Rollin Wild', 'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using ''Content here, content here'', making it look like readable English. Many desktop publishing packages and web page editors now use Lorem Ipsum as their default model text, and a search for ''lorem ipsum'' will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by accident, sometimes on purpose (injected humour and the like).', '', 'https://www.youtube.com/watch?v=yltlJEdSAHw', '', '', '92', 1, '2:02', '', '', '', '', 'https://i3.ytimg.com/vi/yltlJEdSAHw/mqdefault.jpg', 'https://i3.ytimg.com/vi/yltlJEdSAHw/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=yltlJEdSAHw', '1', 87, 0, 0, '2014-05-06 12:59:11', '0', '0', 0, 0, 1, 0, " . $member_id . ", 35),
				(75, 'Amazing nature', 'There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don''t look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn''t anything embarrassing hidden in the middle of text. All the Lorem Ipsum generators on the Internet tend to repeat predefined chunks as necessary, making this the first true generator on the Internet. It uses a dictionary of over 200 Latin words, combined with a handful of model sentence structures, to generate Lorem Ipsum which looks reasonable. The generated Lorem Ipsum is therefore always free from repetition, injected humour, or non-characteristic words etc.', '', 'https://www.youtube.com/watch?v=6v2L2UGZJAM', '', '', '116', 1, '13:29', '', '', '', '', 'https://i3.ytimg.com/vi/6v2L2UGZJAM/mqdefault.jpg', 'https://i3.ytimg.com/vi/6v2L2UGZJAM/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=6v2L2UGZJAM', '1', 40, 0, 0, '2014-05-06 14:24:00', '0', '0', 0, 0, 1, 0, " . $member_id . ", 46),
				(59, 'Wild Country', 'This documentary as well as all of the rest of these documentaries shown here are about important times and figures in history, historic places and people, archaeology, science, conspiracy theories, and education.\r\n\r\nThe Topics of these video documentaries cover just about everything including ancient history, Rome, Greece, Egypt, science, technology, nature, plants, animals, wildlife, environmental issues, global warming, natural disasters, planet earth, the solar system, the universe, modern physics, World wars, battles, military and combat technology, current events, education, biographies, television, archaeology, Illuminati, Area 51, crime, mafia, serial killers, paranormal, supernatural, cults, government cover-ups, the law and legal matters, news and current events, corruption, martial arts, space, aliens, ufos, conspiracy theories, Annunaki, Nibiru, Nephilim, satanic rituals, religion, strange phenomenon, origins of Mankind, monsters, mobsters, time travel', '', 'https://www.youtube.com/watch?v=EpFR_UYhOKY', '', '', '100', 1, '4:20', '', '', '', '', 'https://i3.ytimg.com/vi/EpFR_UYhOKY/mqdefault.jpg', 'https://i3.ytimg.com/vi/EpFR_UYhOKY/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=EpFR_UYhOKY', '1', 6, 0, 0, '2014-05-06 13:30:04', '0', '0', 0, 0, 1, 0, " . $member_id . ", 36),
				(74, 'Best Photos', 'Animals are eukaryotic organisms of the kingdom Animalia or Metazoa. Their body plan eventually becomes fixed as they develop, although some undergo a process of metamorphosis later on in their lives. Most animals are motile, meaning they can move spontaneously and independently. All animals must ingest other organisms or their products for sustenance (see Heterotroph).\r\n\r\nMost known animal phyla appeared in the fossil record as marine species during the Cambrian explosion, about 542 million years ago. Animals are divided into various sub-groups, including birds, mammals, amphibians, reptiles, fish and insects.', '', 'https://www.youtube.com/watch?v=uGACkEOJpR0', '', '', '115', 1, '6:41', '', '', '', '', 'https://i3.ytimg.com/vi/uGACkEOJpR0/mqdefault.jpg', 'https://i3.ytimg.com/vi/uGACkEOJpR0/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=uGACkEOJpR0', '1', 24, 0, 0, '2014-05-06 14:21:45', '0', '0', 0, 0, 1, 0, " . $member_id . ", 45),
				(61, 'Crater Lions', 'The lion (Panthera leo) is one of the four big cats in the genus Panthera and a member of the family Felidae. With some males exceeding 250 kg (550 lb) in weight, it is the second-largest living cat after the tiger. Wild lions currently exist in sub-Saharan Africa and in Asia (where an endangered remnant population resides in Gir Forest National Park in India) while other types of lions have disappeared from North Africa and Southwest Asia in historic times. Until the late Pleistocene, about 10,000 years ago, the lion was the most widespread large land mammal after humans. They were found in most of Africa, across Eurasia from western Europe to India, and in the Americas from the Yukon to Peru. The lion is a vulnerable species, having seen a major population decline in its African range of 30--50% per two decades during the second half of the 20th century. Lion populations are untenable outside designated reserves and national parks. Although the cause of the decline is not fully understood, habitat loss and conflicts with humans are currently the greatest causes of concern. Within Africa, the West African lion population is particularly endangered.', '', 'https://www.youtube.com/watch?v=PvhuzoqsshY', '', '', '102', 1, '1:08', '', '', '', '', 'https://i3.ytimg.com/vi/PvhuzoqsshY/mqdefault.jpg', 'https://i3.ytimg.com/vi/PvhuzoqsshY/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=PvhuzoqsshY', '1', 11, 0, 0, '2014-05-06 13:32:54', '0', '0', 0, 0, 1, 0, " . $member_id . ", 38),
				(62, 'The Jaguar', 'The Jaguar - year of the cat - animals wildlife nature.\r\n\r\nIt is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using ''Content here, content here'', making it look like readable English. Many desktop publishing packages and web page editors now use Lorem Ipsum as their default model text, and a search for ''lorem ipsum'' will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by accident, sometimes on purpose (injected humour and the like).', '', 'https://www.youtube.com/watch?v=dxByg5SKxJA', '', '', '103', 1, '6:53', '', '', '', '', 'https://i3.ytimg.com/vi/dxByg5SKxJA/mqdefault.jpg', 'https://i3.ytimg.com/vi/dxByg5SKxJA/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=dxByg5SKxJA', '1', 7, 0, 0, '2014-05-06 13:33:49', '0', '0', 0, 0, 1, 0, " . $member_id . ", 39),
				(66, 'Great Photos', 'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using ''Content here, content here'', making it look like readable English. Many desktop publishing packages and web page editors now use Lorem Ipsum as their default model text, and a search for ''lorem ipsum'' will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by accident, sometimes on purpose (injected humour and the like).', '', 'https://www.youtube.com/watch?v=r9t_sOsWfJM', '', '', '107', 1, '3:00', '', '', '', '', 'https://i3.ytimg.com/vi/r9t_sOsWfJM/mqdefault.jpg', 'https://i3.ytimg.com/vi/r9t_sOsWfJM/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=r9t_sOsWfJM', '1', 17, 0, 0, '2014-05-06 13:42:35', '0', '0', 0, 0, 1, 0, " . $member_id . ", 42),
				(76, 'Wild America', 'This film features some of the incredible wildlife seen in the American Southwest (Arizona, New Mexico, and California) and Mexico.  Rattlesnakes feature prominently, as well as other reptile species, birds, and sea mammals.  For such seemingly desolate environments (Mojave, Sonoran, and Chihuahuan Deserts), the biodiversity is astounding!', '', 'https://www.youtube.com/watch?v=e1e8rNqpMc4', '', '', '117', 1, '6:34', '', '', '', '', 'https://i3.ytimg.com/vi/e1e8rNqpMc4/mqdefault.jpg', 'https://i3.ytimg.com/vi/e1e8rNqpMc4/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=e1e8rNqpMc4', '1', 97, 0, 0, '2014-05-06 14:25:52', '0', '0', 0, 0, 1, 0, " . $member_id . ", 45),
				(65, 'African Wildlife', 'Our South African represents a sample of the hundreds of species of found in South Africa. (The term ''wildlife'' refers to mammals, birds, fish and reptiles that can be found in the wild). There are 299 mammal species in South Africa, of which 2 are critically endangered, 11 are endangered, 15 are vulnerable, and 13 are near-threatened (conservation status as assessed by the IUCN).', '', 'https://www.youtube.com/watch?v=apNSKQ5kZL0', '', '', '106', 1, '41:34', '', '', '', '', 'https://i3.ytimg.com/vi/apNSKQ5kZL0/mqdefault.jpg', 'https://i3.ytimg.com/vi/apNSKQ5kZL0/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=apNSKQ5kZL0', '1', 9, 0, 0, '2014-05-06 13:39:15', '0', '0', 0, 0, 1, 0, " . $member_id . ", 42),
				(70, 'Emperors of the Ice', 'Wildlife photographer Paul Nicklen dives beneath the Antarctic ice to capture the \"bubbly\" emperor penguins in action, and risks being mistaken for his subjects by some very hungry predators. There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration.', '', 'https://www.youtube.com/watch?v=wr4d2FfivA4', '', '', '111', 1, '20:39', '', '', '', '', 'https://i3.ytimg.com/vi/wr4d2FfivA4/mqdefault.jpg', 'https://i3.ytimg.com/vi/wr4d2FfivA4/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=wr4d2FfivA4', '1', 9, 0, 0, '2014-05-06 13:50:47', '0', '0', 0, 0, 1, 0, " . $member_id . ", 44),
				(69, 'The Call of Everest', 'Fifty years ago, Americans summited Mount Everest for the first time. To celebrate this anniversary, climbers Conrad Anker and Emily Harrington, writer Mark Jenkins, and naturalist Alton Byers meet to discuss the history and future of the world''s highest peak.', '', 'https://www.youtube.com/watch?v=megSEXmV0nQ', '', '', '110', 1, '31:39', '', '', '', '', 'https://i3.ytimg.com/vi/megSEXmV0nQ/mqdefault.jpg', 'https://i3.ytimg.com/vi/megSEXmV0nQ/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=megSEXmV0nQ', '1', 13, 0, 0, '2014-05-06 13:49:28', '0', '0', 0, 0, 1, 0, " . $member_id . ", 43),
				(77, 'Harsh Lighting', 'You''ve probably learned it''s a bad idea to shoot on the beach at noon due to harsh lighting. However, with proper technique using a reflector and the right camera settings, you can get some great shots with soft glow on your model.\r\n\r\nIn this quick tutorial, photographer Matt Hackney teaches how to get that soft look using  a 50mm lens.', '', 'https://www.youtube.com/watch?v=VS4G5SRuLSg', '', '', '118', 1, '2:45', '', '', '', '', 'https://i3.ytimg.com/vi/VS4G5SRuLSg/mqdefault.jpg', 'https://i3.ytimg.com/vi/VS4G5SRuLSg/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=VS4G5SRuLSg', '1', 161, 0, 0, '2014-05-06 14:26:20', '0', '0', 0, 0, 1, 0, " . $member_id . ", 46),
				(78, 'My best trip', 'There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don''t look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn''t anything embarrassing hidden in the middle of text. All the Lorem Ipsum generators on the Internet tend to repeat predefined chunks as necessary, making this the first true generator on the Internet. It uses a dictionary of over 200 Latin words, combined with a handful of model sentence structures, to generate Lorem Ipsum which looks reasonable. The generated Lorem Ipsum is therefore always free from repetition, injected humour, or non-characteristic words etc.', '', 'https://www.youtube.com/watch?v=WtQd6psqgis', '', '', '119', 1, '17:21', '', '', '', '', 'https://i3.ytimg.com/vi/WtQd6psqgis/mqdefault.jpg', 'https://i3.ytimg.com/vi/WtQd6psqgis/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=WtQd6psqgis', '1', 12, 0, 0, '2014-05-06 14:48:17', '0', '0', 0, 0, 1, 0, " . $member_id . ", 47),
				(73, 'Animals Photos', 'They have a few specialized reproductive cells, which undergo meiosis to produce smaller, motile spermatozoa or larger, non-motile ova. These fuse to form zygotes, which develop into new individuals. Many animals are also capable of asexual reproduction. This may take place through parthenogenesis, where fertile eggs are produced without mating, budding, or fragmentation. A zygote initially develops into a hollow sphere, called a blastula, which undergoes rearrangement and differentiation. In sponges, blastula larvae swim to a new location and develop into a new sponge. In most other groups, the blastula undergoes more complicated rearrangement. It first invaginates to form a gastrula with a digestive chamber, and two separate germ layers — an external ectoderm and an internal endoderm. In most cases, a mesoderm also develops between them. These germ layers then differentiate to form tissues and organs.', '', 'https://www.youtube.com/watch?v=QchpUZDDIGg', '', '', '114', 1, '6:41', '', '', '', '', 'https://i3.ytimg.com/vi/QchpUZDDIGg/mqdefault.jpg', 'https://i3.ytimg.com/vi/QchpUZDDIGg/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=QchpUZDDIGg', '1', 10, 0, 0, '2014-05-06 14:13:54', '0', '0', 0, 0, 1, 0, " . $member_id . ", 47),
				(79, 'Strobes & Daylight', 'In today''s Slanted Lens lighting tutorial we are out at the Bonneville Salt Flats. Our lesson will look at how to blend strobes lighting with daylight, so that the image does not look artificially lit. We will look at a method that I use to see what the strobes are doing in daylight and to help you better control them when the modeling lights are not bright enough to guide you. For our shots today we have an old wooden boat as a prop to work with. Out talent, Mary, will be wearing a dark vintage dress and be bare foot in the ice cold water. I wanted her to wear something dark so it will contrast against the light sky. In our last shot we will switch her to a light dress as it gets darker, so it''s easier to get her to stand out against the darker sky. Let''s take a look at shooting strobes in daylight. Keep those cameras rolling and keep on click''n.', '', 'https://www.youtube.com/watch?v=EhhZSFIzfgA', '', '', '120', 1, '6:39', '', '', '', '', 'https://i3.ytimg.com/vi/EhhZSFIzfgA/mqdefault.jpg', 'https://i3.ytimg.com/vi/EhhZSFIzfgA/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=EhhZSFIzfgA', '1', 18, 0, 0, '2014-05-06 14:50:35', '0', '0', 0, 0, 1, 0, " . $member_id . ", 48),
				(80, 'Recorded Webinar', 'On January 22, 2014, Profoto and Mark Wallace did a live webinar on the many different portrait looks you can achieve with just a single light source. Did you miss it? No worries. The webinar was recorded and is now available as a video.\r\n\r\nProfoto hosts free webinars once a month. Click the link to sign up, and we''ll send you a friendly reminder when the next one is about to start.', '', 'https://www.youtube.com/watch?v=2E3Jwgp3MDk', '', '', '121', 1, '56:37', '', '', '', '', 'https://i3.ytimg.com/vi/2E3Jwgp3MDk/mqdefault.jpg', 'https://i3.ytimg.com/vi/2E3Jwgp3MDk/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=2E3Jwgp3MDk', '1', 17, 0, 0, '2014-05-06 14:51:50', '0', '0', 0, 0, 1, 0, " . $member_id . ", 49),
				(81, 'Newfangled Pinup Girl', 'We''ve wanted to do a pinup photo shoot for a long time, but we also wanted to make it stand out from all of the other pinup photos out there by adding a concept to it.', '', 'https://www.youtube.com/watch?v=cBUsA-AoRBg', '', '', '122', 1, '7:47', '', '', '', '', 'https://i3.ytimg.com/vi/cBUsA-AoRBg/mqdefault.jpg', 'https://i3.ytimg.com/vi/cBUsA-AoRBg/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=cBUsA-AoRBg', '1', 50, 0, 0, '2014-05-06 14:55:07', '0', '0', 0, 0, 1, 0, " . $member_id . ", 50),
				(82, 'Bounce Flash', 'There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don''t look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn''t anything embarrassing hidden in the middle of text. All the Lorem Ipsum generators on the Internet tend to repeat predefined chunks as necessary, making this the first true generator on the Internet. It uses a dictionary of over 200 Latin words, combined with a handful of model sentence structures, to generate Lorem Ipsum which looks reasonable. The generated Lorem Ipsum is therefore always free from repetition, injected humour, or non-characteristic words etc.', '', 'https://www.youtube.com/watch?v=xTlBpNrxNLk', '', '', '123', 1, '6:11', '', '', '', '', 'https://i3.ytimg.com/vi/xTlBpNrxNLk/mqdefault.jpg', 'https://i3.ytimg.com/vi/xTlBpNrxNLk/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=xTlBpNrxNLk', '1', 42, 0, 0, '2014-05-07 06:11:47', '0', '0', 0, 0, 1, 0, " . $member_id . ", 51),
				(83, 'Take Pictures at Night', 'There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don''t look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn''t anything embarrassing hidden in the middle of text. All the Lorem Ipsum generators on the Internet tend to repeat predefined chunks as necessary, making this the first true generator on the Internet. It uses a dictionary of over 200 Latin words, combined with a handful of model sentence structures, to generate Lorem Ipsum which looks reasonable. The generated Lorem Ipsum is therefore always free from repetition, injected humour, or non-characteristic words etc.', '', 'https://www.youtube.com/watch?v=V6ypRbPzoPM', '', '', '124', 1, '14:02', '', '', '', '', 'https://i3.ytimg.com/vi/V6ypRbPzoPM/mqdefault.jpg', 'https://i3.ytimg.com/vi/V6ypRbPzoPM/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=V6ypRbPzoPM', '1', 4, 0, 0, '2014-05-07 06:13:49', '0', '0', 0, 0, 1, 0, " . $member_id . ", 52),
				(84, 'Digital photography', 'There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don''t look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn''t anything embarrassing hidden in the middle of text. All the Lorem Ipsum generators on the Internet tend to repeat predefined chunks as necessary, making this the first true generator on the Internet. It uses a dictionary of over 200 Latin words, combined with a handful of model sentence structures, to generate Lorem Ipsum which looks reasonable. The generated Lorem Ipsum is therefore always free from repetition, injected humour, or non-characteristic words etc.', '', 'https://www.youtube.com/watch?v=JwT5WzsaXv0', '', '', '125', 1, '3:31', '', '', '', '', 'https://i3.ytimg.com/vi/JwT5WzsaXv0/mqdefault.jpg', 'https://i3.ytimg.com/vi/JwT5WzsaXv0/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=JwT5WzsaXv0', '1', 12, 0, 0, '2014-05-07 06:15:03', '0', '0', 0, 0, 1, 0, " . $member_id . ", 53),
				(86, 'Love & Marriage', 'Learn how to use your chart to predict when you will fall in love or even get married in this Howcast video featuring famed astrologer Jenny Lynch.\r\n\r\nThere are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don''t look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn''t anything embarrassing hidden in the middle of text. All the Lorem Ipsum generators on the Internet tend to repeat predefined chunks as necessary, making this the first true generator on the Internet.', '', 'https://www.youtube.com/watch?v=ALB6mVmHptM', '', '', '132', 1, '1:34', '', '', '', '', 'https://i3.ytimg.com/vi/ALB6mVmHptM/mqdefault.jpg', 'https://i3.ytimg.com/vi/ALB6mVmHptM/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=ALB6mVmHptM', '1', 19, 0, 0, '2014-05-07 14:38:26', '0', '0', 0, 0, 1, 0, " . $member_id . ", 55),
				(87, 'Chart & Relationships', 'Learn what a composite chart is and what it can tell you about the true purpose of your relationships in this Howcast video featuring astrologer Jenny Lynch.\r\n\r\nThere are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don''t look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn''t anything embarrassing hidden in the middle of text. All the Lorem Ipsum generators on the Internet tend to repeat predefined chunks as necessary, making this the first true generator on the Internet.', '', 'https://www.youtube.com/watch?v=T-gOBMDAPv0', '', '', '133', 1, '1:21', '', '', '', '', 'https://i3.ytimg.com/vi/T-gOBMDAPv0/mqdefault.jpg', 'https://i3.ytimg.com/vi/T-gOBMDAPv0/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=T-gOBMDAPv0', '1', 9, 0, 0, '2014-05-07 14:39:38', '0', '0', 0, 0, 1, 0, " . $member_id . ", 56),
				(88, 'Predicting Compatibility', 'Learn how to predict compatibility with your astrological chart from famed astrologer Jenny Lynch in this Howcast video.\r\n\r\nIt is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using ''Content here, content here'', making it look like readable English.', '', 'https://www.youtube.com/watch?v=ke2y1HfY3T8', '', '', '134', 1, '1:15', '', '', '', '', 'https://i3.ytimg.com/vi/ke2y1HfY3T8/mqdefault.jpg', 'https://i3.ytimg.com/vi/ke2y1HfY3T8/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=ke2y1HfY3T8', '1', 254, 0, 0, '2014-05-07 14:41:00', '0', '0', 0, 0, 1, 0, " . $member_id . ", 57),
				(89, 'First Date Tips', 'Pick up some first date tips for every zodiac sign from famed astrologer Jenny Lynch in this Howcast video.\r\n\r\nIt is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters.', '', 'https://www.youtube.com/watch?v=yicj8wQ2YIo', '', '', '135', 1, '2:01', '', '', '', '', 'https://i3.ytimg.com/vi/yicj8wQ2YIo/mqdefault.jpg', 'https://i3.ytimg.com/vi/yicj8wQ2YIo/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=yicj8wQ2YIo', '1', 587, 0, 0, '2014-05-07 14:41:48', '0', '0', 0, 0, 1, 0, " . $member_id . ", 58),
				(91, 'Christmas in Yellowstone', 'Stretching across more than 2.2 million acres of Wyoming, Montana, and Idaho is one of the greatest expanses of unspoiled nature and wildlife anywhere on Earth — Yellowstone National Park. Designated America''s first national park in 1872, Yellowstone now receives almost three million visitors each year. Yet only a small fraction of those who glimpse the park''s stunning vistas, geological wonders, and animal residents do so during the winter months, a time when nature''s inhospitality is matched only by its serenity.', '', 'https://www.youtube.com/watch?v=xYFZehtdWRs', '', '', '137', 1, '7:43', '', '', '', '', 'https://i3.ytimg.com/vi/xYFZehtdWRs/mqdefault.jpg', 'https://i3.ytimg.com/vi/xYFZehtdWRs/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=xYFZehtdWRs', '1', 9, 0, 0, '2014-05-07 15:13:51', '0', '0', 0, 0, 1, 0, " . $member_id . ", 59),
				(92, 'Adventure', 'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using ''Content here, content here'', making it look like readable English.', '', 'https://www.youtube.com/watch?v=2E9ElqZOlaw', '', '', '138', 1, '3:06', '', '', '', '', 'https://i3.ytimg.com/vi/2E9ElqZOlaw/mqdefault.jpg', 'https://i3.ytimg.com/vi/2E9ElqZOlaw/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=2E9ElqZOlaw', '1', 109, 0, 0, '2014-05-07 15:16:38', '0', '0', 0, 0, 1, 0, " . $member_id . ", 60),
				(93, 'Alaska adventure', 'In 2012 I drove to Alaska to gather animal photos for magazines. \r\nThe adventure was amazing.\r\n\r\nIt is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using ''Content here, content here'', making it look like readable English.', '', 'https://www.youtube.com/watch?v=u93vk9U8XUc', '', '', '139', 1, '14:48', '', '', '', '', 'https://i3.ytimg.com/vi/u93vk9U8XUc/mqdefault.jpg', 'https://i3.ytimg.com/vi/u93vk9U8XUc/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=u93vk9U8XUc', '1', 18, 0, 0, '2014-05-07 15:19:07', '0', '0', 0, 0, 1, 0, " . $member_id . ", 61),
				(94, 'Grizzly Eats My GoPro', 'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using ''Content here, content here'', making it look like readable English.', '', 'https://www.youtube.com/watch?v=DWbXfkNuupM', '', '', '140', 1, '1:20', '', '', '', '', 'https://i3.ytimg.com/vi/DWbXfkNuupM/mqdefault.jpg', 'https://i3.ytimg.com/vi/DWbXfkNuupM/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=DWbXfkNuupM', '1', 20, 0, 0, '2014-05-07 15:20:42', '0', '0', 0, 0, 1, 0, " . $member_id . ", 62),
				(95, 'The Wild Photoshop', 'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using ''Content here, content here'', making it look like readable English.', '', 'https://www.youtube.com/watch?v=L_jcOJb-N9s', '', '', '141', 1, '3:09', '', '', '', '', 'https://i3.ytimg.com/vi/L_jcOJb-N9s/mqdefault.jpg', 'https://i3.ytimg.com/vi/L_jcOJb-N9s/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=L_jcOJb-N9s', '1', 48, 0, 0, '2014-05-07 15:30:05', '0', '1', 1, 0, 1, 0, " . $member_id . ", 63),
				(96, 'Perfect Exposure', 'Join host Joe Brady as he puts the Sekonic meter and the ColorChecker Passport with Sekonic Grey Balance Card to work out in the field to get perfect color and perfect exposure when shooting landscape photography.\r\n\r\nYou will learn the benefits a handheld meter has over your camera''s metering and how creating a custom exposure profile can insure perfect exposures every time. Add to this the ColorChecker Passport''s ability to create a custom color profile for your Raw files and you will have the best starting point possible for your image edits.\r\n\r\nJoe will also explain how to factor in filter compensations and some tips and techniques to get the most tonal range out of your camera''s sensor.', '', 'https://www.youtube.com/watch?v=UUrNGhKS1h8', '', '', '144', 1, '1:11:47', '', '', '', '', 'https://i3.ytimg.com/vi/UUrNGhKS1h8/mqdefault.jpg', 'https://i3.ytimg.com/vi/UUrNGhKS1h8/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=UUrNGhKS1h8', '1', 55, 0, 0, '2014-05-08 12:01:27', '1', '1', 0, 0, 1, 0, " . $member_id . ", 64),
				(97, 'Create Color Depth', 'Animals are eukaryotic organisms of the kingdom Animalia or Metazoa. Their body many desktop packages and web page now use Lorem as model text and a search for ''lorem ipsum'' will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by accident, sometimes on purpose (injected humour and the like).', '', 'https://www.youtube.com/watch?v=MaLELeXtI24', '', '', '145', 1, '17:55', '', '', '', '', 'https://i3.ytimg.com/vi/MaLELeXtI24/mqdefault.jpg', 'https://i3.ytimg.com/vi/MaLELeXtI24/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=MaLELeXtI24', '1', 188, 0, 0, '2014-05-08 12:02:31', '0', '1', 1, 0, 1, 0, " . $member_id . ", 65),
				(98, 'Winter Landscapes', 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sit amet fringilla orci. Aenean molestie dolor nibh, non accumsan orci fermentum vitae. Proin bibendum at diam tincidunt fermentum. Nunc venenatis volutpat erat, sit amet scelerisque libero vestibulum id. Maecenas vitae porttitor dolor. Mauris gravida lacus quis elit eleifend auctor sit amet sed orci. Ut nulla risus, luctus sit amet commodo eget, sollicitudin in magna. Etiam placerat eros ante, vitae interdum mi semper non. Vestibulum id fermentum odio.', '', 'https://www.youtube.com/watch?v=Q8Lg8e9hML4', '', '', '146', 1, '7:13', '', '', '', '', 'https://i3.ytimg.com/vi/Q8Lg8e9hML4/mqdefault.jpg', 'https://i3.ytimg.com/vi/Q8Lg8e9hML4/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=Q8Lg8e9hML4', '1', 118, 0, 0, '2014-05-08 12:06:09', '0', '0', 0, 0, 1, 0, " . $member_id . ", 66),
				(99, 'Winter in Bandon', 'Bandon is a delightfully charming Southern Oregon coastal town located 90 miles north of the California border. Its allure of nature''s beauty is undeniable even during the winter months.\r\n\r\nLorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sit amet fringilla orci. Aenean molestie dolor nibh, non accumsan orci fermentum vitae. Proin bibendum at diam tincidunt fermentum. Nunc venenatis volutpat erat, sit amet scelerisque libero vestibulum id. Maecenas vitae porttitor dolor. Mauris gravida lacus quis elit eleifend auctor sit amet sed orci. Ut nulla risus, luctus sit amet commodo eget, sollicitudin in magna. Etiam placerat eros ante, vitae interdum mi semper non. Vestibulum id fermentum odio.', '', 'https://www.youtube.com/watch?v=NG34swnz7xA', '', '', '147', 1, '4:31', '', '', '', '', 'https://i3.ytimg.com/vi/NG34swnz7xA/mqdefault.jpg', 'https://i3.ytimg.com/vi/NG34swnz7xA/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=NG34swnz7xA', '1', 6, 0, 0, '2014-05-08 12:06:58', '0', '0', 0, 0, 1, 0, " . $member_id . ", 67),
				(100, 'Earth of the year 2013', 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sit amet fringilla orci. Aenean molestie dolor nibh, non accumsan orci fermentum vitae. Proin bibendum at diam tincidunt fermentum. Nunc venenatis volutpat erat, sit amet scelerisque libero vestibulum id. Maecenas vitae porttitor dolor. Mauris gravida lacus quis elit eleifend auctor sit amet sed orci. Ut nulla risus, luctus sit amet commodo eget, sollicitudin in magna. Etiam placerat eros ante, vitae interdum mi semper non. Vestibulum id fermentum odio.', '', 'https://www.youtube.com/watch?v=xno0yCTURmY', '', '', '148', 1, '10:01', '', '', '', '', 'https://i3.ytimg.com/vi/xno0yCTURmY/mqdefault.jpg', 'https://i3.ytimg.com/vi/xno0yCTURmY/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=xno0yCTURmY', '1', 8, 0, 0, '2014-05-08 12:18:25', '0', '0', 0, 0, 1, 0, " . $member_id . ", 68),
				(101, 'Amazing Sunset Photos', 'For much more than this, you can visit our global channel\r\nDont forget to subscribe our daily Channel.. Nothing says ''paradise'' quite like a great sunset photo. Capturing the mix of reds, yellows, oranges, and magentas in the evening sky is still one of my favo', '', 'https://www.youtube.com/watch?v=cFV-HcMEwXQ', '', '', '149', 1, '11:13', '', '', '', '', 'https://i3.ytimg.com/vi/cFV-HcMEwXQ/mqdefault.jpg', 'https://i3.ytimg.com/vi/cFV-HcMEwXQ/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=cFV-HcMEwXQ', '1', 377, 0, 0, '2014-05-08 12:19:37', '0', '1', 1, 0, 1, 0, " . $member_id . ", 69),
				(102, 'Unbelievable ', 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sit amet fringilla orci. Aenean molestie dolor nibh, non accumsan orci fermentum vitae. Proin bibendum at diam tincidunt fermentum. Nunc venenatis volutpat erat, sit amet scelerisque libero vestibulum id. Maecenas vitae porttitor dolor. Mauris gravida lacus quis elit eleifend auctor sit amet sed orci. Ut nulla risus, luctus sit amet commodo eget, sollicitudin in magna. Etiam placerat eros ante, vitae interdum mi semper non. Vestibulum id fermentum odio.', '', 'https://www.youtube.com/watch?v=VlhJm_KkKEg', '', '', '150', 1, '1:32', '', '', '', '', 'https://i3.ytimg.com/vi/VlhJm_KkKEg/mqdefault.jpg', 'https://i3.ytimg.com/vi/VlhJm_KkKEg/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=VlhJm_KkKEg', '1', 20, 0, 0, '2014-05-08 12:21:11', '0', '0', 0, 0, 1, 0, " . $member_id . ", 70),
				(103, 'The Moon', 'The Moon is the only natural satellite of the Earth and the fifth largest moon in the Solar System. The Moon is in synchronous rotation with Earth, always showing the same face with its near side marked by dark volcanic maria that fill between the bright ancient crustal highlands and the prominent impact craters. It is the brightest object in the sky after the Sun, although its surface is actually dark, with a reflectance just slightly higher than that of worn asphalt. Its prominence in the sky and its regular cycle of phases have, since ancient times, made the Moon an important cultural influence on language, calendars, art and mythology. The Moon''s gravitational influence produces the ocean tides and the minute lengthening of the day. Dr. Eugene Shoemaker, Full moon, magnetic field, Neil Armstrong, Apollo 8, Apollo 11, luna, NASA, craters, astronomy, astrology, quantum, space', '', 'https://www.youtube.com/watch?v=H5yrwuDDgYE', '', '', '151', 1, '2:56', '', '', '', '', 'https://i3.ytimg.com/vi/H5yrwuDDgYE/mqdefault.jpg', 'https://i3.ytimg.com/vi/H5yrwuDDgYE/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=H5yrwuDDgYE', '1', 19, 0, 0, '2014-05-08 12:22:17', '0', '0', 0, 0, 1, 0, " . $member_id . ", 71),
				(104, 'People will live on Mars', 'Animals are eukaryotic organisms of the kingdom Animalia or Metazoa. Their body more than 100,000 people are eager to make themselves at home on another planet. They''ve applied for a one-way trip to Mars, hoping to be chosen to spend the rest of their lives on uncharted territory, according to an organization planning the manned missions.', '', 'https://www.youtube.com/watch?v=icN29cdmw_s', '', '', '152', 1, '4:07', '', '', '', '', 'https://i3.ytimg.com/vi/icN29cdmw_s/mqdefault.jpg', 'https://i3.ytimg.com/vi/icN29cdmw_s/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=icN29cdmw_s', '1', 15, 0, 0, '2014-05-08 12:23:06', '0', '0', 0, 0, 1, 0, " . $member_id . ", 72),
				(106, 'Colonize Mars', 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sit amet fringilla orci. Aenean molestie dolor nibh, non accumsan orci fermentum vitae. Proin bibendum at diam tincidunt fermentum. Nunc venenatis volutpat erat, sit amet scelerisque libero vestibulum id. Maecenas vitae porttitor dolor. Mauris gravida lacus quis elit eleifend auctor sit amet sed orci. Ut nulla risus, luctus sit amet commodo eget, sollicitudin in magna. Etiam placerat eros ante, vitae interdum mi semper non. Vestibulum id fermentum odio.', '', 'https://www.youtube.com/watch?v=B-rEDyCo0ao', '', '', '154', 1, '44:26', '', '', '', '', 'https://i3.ytimg.com/vi/B-rEDyCo0ao/mqdefault.jpg', 'https://i3.ytimg.com/vi/B-rEDyCo0ao/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=B-rEDyCo0ao', '1', 8, 0, 0, '2014-05-08 13:28:08', '0', '0', 0, 0, 1, 0, " . $member_id . ", 73),
				(107, 'The Mars Underground', 'This film captures the spirit of Mars pioneers who refuse to let their dreams be put on hold by a slumbering space program. Their passionate urge to walk the soil of an alien world is infectious and inspirational. This film is the manifesto of the new space revolution.', '', 'https://www.youtube.com/watch?v=dRxskmMMnfc', '', '', '155', 1, '1:13:53', '', '', '', '', 'https://i3.ytimg.com/vi/dRxskmMMnfc/mqdefault.jpg', 'https://i3.ytimg.com/vi/dRxskmMMnfc/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=dRxskmMMnfc', '1', 7, 0, 0, '2014-05-08 13:28:32', '0', '0', 0, 0, 1, 0, " . $member_id . ", 74),
				(108, 'Mars Exploration Rover', 'NASA''s Mars Exploration Rover Mission (MER) is an ongoing robotic space mission involving two rovers, Spirit and Opportunity, exploring the planet Mars. It began in 2003 with the sending of the two rovers—MER-A Spirit and MER-B Opportunity—to explore the Martian surface and geology.\r\n\r\nThe mission''s scientific objective was to search for and characterize a wide range of rocks and soils that hold clues to past water activity on Mars. The mission is part of NASA''s Mars Exploration Program, which includes three previous successful landers: the two Viking program landers in 1976 and Mars Pathfinder probe in 1997.ence platform.', '', 'https://www.youtube.com/watch?v=7UUPiCVbZhE', '', '', '156', 1, '0:56', '', '', '', '', 'https://i3.ytimg.com/vi/7UUPiCVbZhE/mqdefault.jpg', 'https://i3.ytimg.com/vi/7UUPiCVbZhE/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=7UUPiCVbZhE', '1', 7, 0, 0, '2014-05-08 13:29:09', '0', '0', 0, 0, 1, 0, " . $member_id . ", 75),
				(109, '7 Minutes of Terror', 'NASA''s Curiosity rover is a 1-ton robot that will make an unprecedented Mars landing on Aug. 5, 2012. See how the risky maneuver will keep rover team members in suspense for 7 fateful minutes.', '', 'https://www.youtube.com/watch?v=VlhJm_KkKEg', '', '', '157', 1, '1:32', '', '', '', '', 'https://i3.ytimg.com/vi/VlhJm_KkKEg/mqdefault.jpg', 'https://i3.ytimg.com/vi/VlhJm_KkKEg/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=VlhJm_KkKEg', '1', 78, 0, 0, '2014-05-08 13:30:08', '0', '0', 0, 0, 1, 0, " . $member_id . ", 76),
				(110, 'Discovery Space', 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sit amet fringilla orci. Aenean molestie dolor nibh, non accumsan orci fermentum vitae. Proin bibendum at diam tincidunt fermentum. Nunc venenatis volutpat erat, sit amet scelerisque libero vestibulum id. Maecenas vitae porttitor dolor. Mauris gravida lacus quis elit eleifend auctor sit amet sed orci. Ut nulla risus, luctus sit amet commodo eget, sollicitudin in magna. Etiam placerat eros ante, vitae interdum mi semper non. Vestibulum id fermentum odio.', '', 'https://www.youtube.com/watch?v=tMVDWvHlrRc', '', '', '158', 1, '1:4:31', '', '', '', '', 'https://i3.ytimg.com/vi/tMVDWvHlrRc/mqdefault.jpg', 'https://i3.ytimg.com/vi/tMVDWvHlrRc/maxresdefault.jpg', '0', 'https://www.youtube.com/watch?v=tMVDWvHlrRc', '1', 113, 0, 0, '2014-05-08 13:31:49', '0', '0', 0, 0, 1, 0, " . $member_id . ", 77),
				(120, 'Holy Monks ', 'A shaolin master has the task of preparing two antagonic disciples with the same goal: turn them into good warriors who are always willing to help the weakest.\r\n\r\nIt is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using ''Content here, content here'', making it look like readable English.', '', 'https://www.youtube.com/watch?v=G2hH1mwCYzE', '', '', '544', 1, '3:30', '', '', '', '', 'https://i3.ytimg.com/vi/G2hH1mwCYzE/mqdefault.jpg', 'https://i3.ytimg.com/vi/G2hH1mwCYzE/maxresdefault.jpg', '', 'https://www.youtube.com/watch?v=G2hH1mwCYzE', '0', 128, 0, 0, '2014-07-14 14:25:36', '', '', 0, 0, 1, 0, " . $member_id . ", 78);");
...
function vh_video_licence() {
	...
	$main_notice = get_option('snaptube_rating_notice');
	if ( !$main_notice ) {
		update_option('snaptube_rating_notice', time());
	}
}

function snaptube_admin_rating_notice() {
	$user = wp_get_current_user();
	if ( get_option('snaptube_rating_notice') != 'hide' && time() - get_option('snaptube_rating_notice') > 864000 ) { ?>
		<div class="snaptube-rating-notice">
			<span class="snaptube-notice-left">
				<img src="<?php echo get_template_directory_uri(); ?>/images/logo-square.png" alt="">
			</span>
			<div class="snaptube-notice-center">
				<p>Hi there, <?php echo $user->data->display_name; ?>, we noticed that you've been using Snaptube for a while now.</p>
				<p>We spent many hours developing this awesome theme for you and we would appriciate if you supported us by rating our theme!</p>
			</div>
			<div class="snaptube-notice-right">
				<a href="https://themeforest.net/downloads" class="button button-primary button-large snaptube-rating-rate">Rate at ThemeForest</a>
				<a href="javascript:void(0)" class="button button-large preview snaptube-rating-dismiss">No, thanks</a>
			</div>
			<div class="clearfix"></div>
		</div>
	<?php }
}
add_action( 'admin_notices', 'snaptube_admin_rating_notice' );

function snaptube_dismiss_rating_notice() {
	update_option('snaptube_rating_notice', 'hide');

	die(0);
}
add_action( 'wp_ajax_snaptube_dismiss_notice', 'snaptube_dismiss_rating_notice' );

function snaptube_reload_images_func() {
	global $wpdb;
	$video_id = intval( $_POST['video_id'] );
	$video_data = $wpdb->get_results('SELECT slug FROM '.$wpdb->prefix.'hdflvvideoshare WHERE vid="'.$video_id.'"');
	if ( !empty($video_data) ) {
		$attached_images = get_attached_media( 'image', $video_data['0']->slug );
		if ( !empty($attached_images) ) {
			foreach ($attached_images as $image_value) {
				wp_delete_attachment( $image_value->ID, true );
			}
			delete_post_meta($video_data['0']->slug, 'video_sd_image');
			delete_post_meta($video_data['0']->slug, 'video_mq_image');
		}
	}
...
add_action( 'wp_ajax_snaptube_reload_images', 'snaptube_reload_images_func' );

function snaptube_theme_activated() {
	if ( !get_option('snaptube_likes_saved') ) {
		update_option('snaptube_likes_saved', time());

		global $wpdb;
		$old_like_table = $wpdb->prefix.'like_dislike_counters';
		if ( $wpdb->get_var("SHOW TABLES LIKE '$old_like_table'") == $old_like_table ) { // Old values exist, so save them into the post meta
			...
			if ( !empty($old_like_values) ) {
				foreach ($old_like_values as $like_values) {
					update_post_meta( $like_values->post_id, 'snaptube_post_likes', $like_values->ul_value );
				}
			}
		}
	}

	if ( !get_option('snaptube_table_altered') ) {
		global $wpdb;
		$ads_table = $wpdb->prefix.'hdflvvideoshare_vgads';
		if ( $wpdb->get_var("SHOW TABLES LIKE '$ads_table'") == $ads_table ) {
			$wpdb->query("ALTER TABLE $ads_table CHANGE file_path file_path VARCHAR(200) CHARACTER SET utf8 COLLATE utf8_general_ci NULL");
			$wpdb->query("ALTER TABLE $ads_table CHANGE title title VARCHAR(200) CHARACTER SET utf8 COLLATE utf8_general_ci NULL");
			$wpdb->query("ALTER TABLE $ads_table MODIFY impressionurl TEXT NULL");
			$wpdb->query("ALTER TABLE $ads_table MODIFY clickurl TEXT NULL");
			update_option('snaptube_table_altered', 'true');
		}
		
	}
}
add_action('after_switch_theme', 'snaptube_theme_activated');

function snaptube_deactivate_likes() {
	deactivate_plugins( 'like-dislike-counter-for-posts-pages-and-comments/main.php' );
}
add_action('admin_init', 'snaptube_deactivate_likes');