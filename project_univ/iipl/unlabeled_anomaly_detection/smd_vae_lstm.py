...
# Data Setting
file_path = os.path.join(os.getcwd(), "data/smd_total.csv")
total_data = pd.read_csv(file_path)
# total_data = total_data.loc[:, ["AccZ", "Str1", "Str2", "Str3"]]
# total_data = total_data.loc[:, ["src1", "src2", "tgt"]]
n_feature = (total_data.shape[-1]-1)
n_past = 5
...
minmax_scaler2 = minmax_scaler.fit(np.array(total_data.tgt[:]).reshape(-1, 1))
total_data_scaled2 = minmax_scaler2.transform(np.array(total_data.tgt[:]).reshape(-1, 1))


train_valid_split = int(len(total_data_scaled) * 0.5)   #argparse
df_train = total_data_scaled[:-train_valid_split]
df_valid = total_data_scaled[-train_valid_split:]

train_data = CustomDataset(df_train, n_past, 1)     #argparse
valid_data = CustomDataset(df_valid, n_past, 1)
...
model = VAELSTM(n_features=n_feature, n_past=n_past, n_future=1, n_layers=8, hidden_size=512, embed_size=256, dropout=0.1)
model.to(device)
...
for epoch in range(start_epoch, epochs+1):
    ...
    data_path = os.path.join(os.getcwd(), "data", "figure_smd_vae-lstm")
