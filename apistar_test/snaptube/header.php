...
<!--<![endif]-->
	<head>
		...
		<?php
			...
			// Get theme logo
			$logo = str_replace('http://', '//', get_option('vh_sitelogo'));
			$logo = str_replace('https://', '//', $logo);
			if($logo == false || $logo == '' ) {
	...
	<body <?php body_class($vh_class.$shadows); ?>>
		<?php if ( $_SERVER['SERVER_NAME'] == 'cohhe.com' ) { ?>
			<a href="//themeforest.net/item/snaptube-premium-video-wordpress-theme/8026657" target="_blank" id="buy-now-ribbon"></a>
		<?php } ?>
