...
$entry_utility .= '<div class="entry-top-utility">';
	if ( 'post' == get_post_type() ) {
		
		$entry_utility .= '<span class="post_likes">' . snaptube_like_button( get_the_ID() ) . '</span>';
...
?>
<div class="entry no_left_margin first-entry <?php echo $is_author_desc; ?> <?php if ( !isset($img[0]) ) { echo ' no-image'; } ?><?php echo (LAYOUT != 'sidebar-no') ? ' span9' : ' span12'; ?>">
			...
			<?php if ( get_post_thumbnail_id() != "" ): ?>
				...
				$tc = wp_count_comments($post_id); 
					...
					if ( function_exists('snaptube_like_button') ) {
						echo '<span class="blog_likes icon-heart">' . snaptube_like_count( $post_id ) . '</span>';
					}
				?>
				</div>
			<?php endif ?>
